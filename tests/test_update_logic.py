...
class UpdateLogicTests(unittest.TestCase):
    ...
    def test_first_install_when_no_install_detected(self) -> None:
        decision = decide_next_state(LauncherState(), None, None, False)
    ...
    def test_installed_current_ready_to_launch(self) -> None:
        ...
        decision = decide_next_state(state, self._install(), self._release("AntiAmend-current"), True)
    ...
    def test_installed_outdated_ready_to_update(self) -> None:
        ...
        decision = decide_next_state(state, self._install(), self._release("AntiAmend-new"), True)
        self.assertEqual(decision.ui_state, "ready_to_update")
        self.assertEqual(decision.action_label, "Update")
        self.assertTrue(decision.local_install_complete)

    def test_installed_current_with_obsolete_ix_files_ready_to_update(self) -> None:
        state = LauncherState(installed_release="AntiAmend-current")
        decision = decide_next_state(
            state,
            self._install(),
            self._release("AntiAmend-current"),
            True,
            cleanup_needed=True,
        )
        self.assertEqual(decision.ui_state, "ready_to_update")
        self.assertEqual(decision.action_label, "Update")
        self.assertEqual(decision.reason, "Cleanup needed.")
        self.assertTrue(decision.local_install_complete)

    def test_saved_state_with_missing_core_files_ready_to_install(self) -> None:
        state = LauncherState(installed_release="AntiAmend-current")
        decision = decide_next_state(state, self._install(), self._release("AntiAmend-current"), False)
        self.assertEqual(decision.ui_state, "ready_to_install")
